...
#pragma omp parallel default (none) shared(block_num, iProc, nProc, from, recv_flag, \
	...
	int const tid = omp_get_thread_num();  
        ...
        if (tid == 0)
	  {
	    ...
	    MPI_Request send_req[nProc-1];
	    int k;
	    // post all receives
	    for (k = 0; k < nProc-1; ++k)
	      {
		const int source = from[iProc][k];
		const int len = mSize * mSize; 
		const int soffset = source * mSize * mSize;
		MPI_Irecv(&(work_array[soffset])
			  , len
			  , MPI_DOUBLE
			  , source
			  , DATAKEY
	    ...
	    for (k = 0; k < nProc-1; ++k)
	      {
		const int target = from[iProc][k];
		const int len = mSize * mSize; 
		const int toffset = target * mSize * mSize;
		MPI_Isend(&(source_array[toffset])
			  , len
			  , MPI_DOUBLE
			  , target
			  , DATAKEY
	    ...
	    // wait for any receive and notify 
	    for (k = 0; k < nProc-1; ++k)
	      {
		int id = -1;
		MPI_Waitany(nProc-1
		...
		ASSERT(id >= 0 && id < nProc -1);      

		// flag received buffer 
		const int source = from[iProc][id];
		increment_ctr(&recv_flag[source].stage);		
