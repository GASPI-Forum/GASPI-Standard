...
Note! For now, the sum of the sizes of \gaspitype{notification} and
...
typedef struct {
  ...
  gaspi_notification_id_t    notification_num;
...
getter routines (\ref{sec:getterfn}) after the successful program initialization. The values of the configuration
...
In case that the the connection has been disconnected already,
e.g. by the connection partner, the return value is \GASPISUCC{}.
...
by invocation of \gaspifunction{state\_vec\_get} gives information
whether the remote \GASPI{} process is still alive.
...
\subsection{Group destruction}

\subsubsection{\gaspifunction{group\_destroy}}

The \gaspifunction{group\_destroy} procedure
is a \gaspisemantic{synchronous} \gaspisemantic{local}
\gaspisemantic{blocking} procedure which destroys
a given group.
...
\begin{verbatim}
gaspi_return_t
gaspi_group_destroy (gaspi_group_t group)
\end{verbatim}
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{in}{group}{the group to be destroyed}
\parameterlistend
\executionphasework
\FStdRetDescNOTimeout
\end{FDef}
...
After successful procedure completion, i.\,e.\ return value \GASPISUCC{},
\parameter{group} is destroyed and cannot be used.

In case of error, the return value is \GASPIGERR{}.

Invoking \gaspifunction{group\_destroy} in any other execution phase than the
working phase yields undefined behavior.

\begin{implementoradvice}
If the parameter \parameter{build\_infrastructure} is not set, the procedure \gaspifunction{group\_destroy} must disconnect
...
\subsection{Segment destruction}

\subsubsection{\gaspifunction{segment\_destroy}}

The \gaspisemantic{synchronous} \gaspisemantic{local}
\gaspisemantic{blocking} procedure \gaspifunction{segment\_destroy} 
...
\begin{verbatim}
gaspi_return_t
gaspi_segment_destroy (gaspi_segment_id_t segment_id)
\end{verbatim}
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{in}{segment\_id}{ The segment ID to be destroyed. }
\parameterlistend
\executionphasework
\FStdRetDescNOTimeout
\end{FDef}

\gaspifunction{segment\_destroy} releases the resources of the segment
which is referenced by the \parameter{segment\_id} identifier.

After successful procedure completion, i.\,e.\ return value \GASPISUCC{},
the segment is destroyed and the resources are released. It is a failure to use the segment for communication
between two \GASPI{} processes after \gaspifunction{destroy} has been called.

In case of return value \GASPIGERR{}, the segment destruction failed. The segment
is in an undefined state. It cannot be used locally or
for passive communication. Furthermore, the segment cannot be used for one sided
communication requests which are invoked by other remote processes.

Invoking \gaspifunction{segment\_destroy} in any other execution
phase than the working phase yields undefined behavior.
...
% \subsubsection{\gaspifunction{segment\_destroy}}
%
% \gaspifunction{segment\_destroy} is a \gaspisemantic{synchronous} \gaspisemantic{collective}
...
%
%
% \gaspifunction{segment\_destroy} destroys and deregister the segment that is referenced by
% the \parameter{segment\_id} identifier. \gaspifunction{segment\_destroy} makes the segment
% referenced by the \parameter{segment\_id} identifier visible and accessible to
% all of the \GASPI{} processes forming the group \parameter{group}.
%
...
% \end{itemize}
%
% In case of return value \GASPIGERR, the segment destruction failed in one of the above progress
% steps.
%
% In case of the latter two return values, a check of the state vector
% by invocation of \gaspifunction{state\_vec\_get} gives information
...
%
% Invoking
% \gaspifunction{segment\_destroy} in any other execution phase than the working
% phase yields undefined behavior.
...
\parameter{segment\_max} in the configuration structure which is retrieved to \gaspifunction{proc\_init} at
...
Invoking
\gaspifunction{segment\_num} in any other execution phase than the working phase
yields undefined behavior.
...
\parameterlistitem{in}{notification\_id}{the remote notification ID where the notification value is sent to}
\parameterlistitem{in}{notification\_val}{the notification value to be sent}
\parameterlistitem{in}{queue}{the queue to be used}
\parameterlistitem{in}{timeout}{the timeout}
\parameterlistend
\executionphasework
...
has even arrived at the remote side.
\end{useradvice}
...
% 
% \begin{todo}
% \item destroy \gaspifunction{notify\_wait}
...
\begin{useradvice}
  ...
  inspecting only one notification
  is the following: The remote side uses a \gaspifunction{write} call
  followed by a subsequent call of \gaspifunction{notify} posted to
  the same queue and the same destination rank. Since the standard
  preserves the order of the messages on the remote side, one can
...
\parameterlistitem{in}{offset\_remote}{the remote offset to be written to}
\parameterlistitem{in}{size}{the size of the message to be sent}
\parameterlistitem{in}{notification\_id}{the remote notification ID where the notification is sent to}
\parameterlistitem{in}{notification\_value}{the value of the notification to be sent}
\parameterlistitem{in}{queue}{the queue to be used}
\parameterlistitem{in}{timeout}{the timeout}
\parameterlistend
\executionphasework
...
\parameterlistitem{in}{offset\_remote[num]}{list of remote offsets to be written to}
\parameterlistitem{in}{size[num]}{list of sizes of the messages to be sent}
\parameterlistitem{in}{notification\_id}{the remote notificatione ID where the notification is sent to}
\parameterlistitem{in}{notification\_value}{the value of the notification to be sent}
\parameterlistitem{in}{queue}{the queue to be used}
\parameterlistitem{in}{timeout}{the timeout}
\parameterlistend
...
The \GASPI{} Standard provides getter functions for all entries in the \GASPI{} configuration.
...
the parameter is set to the provided parameter. Otherwise, the parameter is set to the minimum compliant 
with the system capabilities. The  values of the parameters  the \GASPI{} configuration are implementation specific.
...
Invoking these getter functions in any other execution phase than the working phase
yields undefined behavior.
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{out}{notification\_max}{the maximum number of available notification}
\parameterlistend
\executionphasework
\FStdRetDescNOTimeout
\end{FDef}
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{out}{network\_type}{the choosen network type}
\parameterlistend
\executionphasework
\FStdRetDescNOTimeout
\end{FDef}
