...
\item Efficient one sided asynchronous remote read/write operations.
...
\item Collective operations for groups of processes.
...
the coorresponding notification has arrived on the remote process, the data from the
earlier posted request to the same process hs alsoe arrived on the remote
side.
...
gaspi_allreduce_user ( buffer_send
    ...
    TYPE(gaspi_state_t), INTENT(IN) :: reduce_state
    TYPE(gaspi_group_t), INTENT(IN) :: group
    TYPE(gaspi_timeout_t), INTENT(IN) :: timeout
    TYPE(gaspi_return_t), INTENT(OUT) :: ireturn
...
gaspi_reduce_operation ( operand_one
    ...
    TYPE(gaspi_state_t), INTENT(IN) :: state
    TYPE(gaspi_timeout_t), INTENT(IN) :: timeout
    TYPE(gaspi_return_t), INTENT(OUT) :: ireturn
\end{verbatim}
\end{FDefSign}
