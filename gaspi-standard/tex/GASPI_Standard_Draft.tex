...
\GASPI{} provides atomic operations for integral types, i.\,e. such variables can
be manipulated atomically without fear of preemption
...
\begin{TDef}
\TDefDef{\gaspitype{atomic\_value}}
\TDefDesc{The \GASPI{} global atomic value type. An atomic value is unsigned and its maximum value can be queried using \gaspifunction{atomic\_max}.}
\end{TDef}
...
(Multiple Program, Multiple Data) style program execution.
Hence, either a single program or different programs can be
started on the computational units. How
a \GASPI{} application is started and initialized is implementation specific.
...
globally accessable memory regions. In general,
...
some of the parameters are set by the program and assigned with \gaspifunction{config\_set},
...
getter routines (see Sect.~\ref{sec:getterfn}) after the successful program initialization. The values of the configuration
...
which is the \gaspitype{state} type. \gaspitype{state} can have one of two values:
...
Allocation of segments in \GASPI{} allows for various so-called policies.
The default policy in a cc-numa mode for example might be an allocation 
of socket-local memory, a different policy might allow to map GPGPU memory
into the main memory of the host and yet another policy might allow for a direct access
of external non-volatile RAM.

The \parameter{alloc\_policy}  is used to pass an allocation policy. The default allocation policy behavior is left to the implementation.
The default allocation parameter is GASPI\_ALLOC\_DEFAULT.
...
the correspondent \gaspifunction{wait} returns with \GASPISUCC{}. For coompleted  \gaspifunction{read}
...
\begin{verbatim}
gaspi_return_t
gaspi_network_type (gaspi_atomic_value_t *max_value)
\end{verbatim}
\end{FDefSign}

\begin{FDefSign}
\begin{verbatim}
gaspi_network_type ( max_value
                   , ireturn 
                   ) BIND(C)
