...
typedef struct {
  ...
  gaspi_number_t     notify_flag_num;
  ...
  gaspi_size_t       allreduce_buf_size_max;
...
There are procedures to query for the state of the communication partners
...
The \GASPI{} memory segments are thus an abstraction that representing
...
sIt is an \gaspisemantic{asynchronous} \gaspisemantic{non-local}
\gaspisemantic{time-based blocking} procedure.
...
\gaspifunction{write\_waitsome} is the correspondent wait procedure
...
The queue size is set to set by a successful call to \gaspifunction{passive\_wait}.
...
is privided through three operations: set, fetch-and-add and compare-and-swap.
...
\begin{verbatim}
gaspi_return_t
gaspi_queue_num (gaspi_number_t queue_max)
\end{verbatim}
\end{FDefSign}
\parameterlistbegin
...
\begin{verbatim}
gaspi_return_t
gaspi_notification_num (gaspi_number_t notification_max)
\end{verbatim}
\end{FDefSign}
\parameterlistbegin
