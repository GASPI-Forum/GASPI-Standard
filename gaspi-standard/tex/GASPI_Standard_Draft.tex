...
\newcommand{\GASPISUCCTXT}{operation has successfully returned}
...
\newenvironment{implementoradvice}{\begin{flushright}\begin{minipage}[t]{0.9\linewidth}\emph{Implementor advice:}}{\finalsymb{\emph{Implementor advice end}}\end{minipage}\end{flushright}}
\newenvironment{useradvice}{\begin{flushright}\begin{minipage}[t]{0.9\linewidth}\emph{User advice:}}{\finalsymb{\emph{User advice end}}\end{minipage}\end{flushright}}
...
, morekeywords={gaspi_rank_t,gaspi_barrier,gaspi_timeout_t,gaspi_return_t,gaspi_configuration_t,gaspi_proc_init,gaspi_proc_term,gaspi_proc_kill
  ...
  ,gaspi_number_t,gaspi_flag_t,gaspi_time_t
  ,gaspi_counter_compare_swap,gaspi_counter_reset
...
The standard guarantees fairness of transfers posted to
different queues, i.\,e.\ no queue should starve to see its
communication requests complete.

Furthermore, the order of messages addressing the same remote \GASPI{}
...
communication patterns where the sender is unknown (ie. it can be any
...
\insertlisting{ex.passive.pseudo}{Single consumer and multiple
  producers using passive communication. The prodcuer transfers a data
  paket while producing the next data paket, thus overlapping
...
The standard guarantees fairness, i.\,e.\ no process should starve to see
its atomic operation complete.
...
collective with respect to a group of ranks. They are also exclusive
per group, i.\,e.\ only one collective operation of a specific type can
run at a time. Two allreduce for one group can not run at the same
time. However, for example, an allreduce and a barrier can run at the
same time.
...
In principle, \GASPI{} procedures have three general return values:
...
and it should be used to detect failures on remote processes.
...
\begin{TDef}
\TDefDefn{int}{\gaspitype{flag}}
\TDefDesc{The \GASPI{} flag type.}
\end{TDef}

\begin{TDef}
\TDefDefn{unsigned short}{\gaspitype{flag\_id}}
\TDefDesc{The \GASPI{} flag ID type.}
\end{TDef}
...
Note! For now, the sum of the sizes of \gaspitype{flag} and
...
\TDefDesc{A type that is used to count elements. That could be numbers
  of queues as well as the depth of individual queues.}
\end{TDef}
...
\TDefDesc{A type that can point to some memory.}
\end{TDef}
...
\item \textbf{Initialization}
  \begin{description}
%   \item Init communication infrastructure
  \item Initialization of the runtime environment
%   \item Create communication infrastructure
%   \item Create groups (optional)
%   \item Create segments (optional).
  \end{description}
\item \textbf{Working}
  \begin{description}
...
exception for pre-initialization routines for environment checking and
...
\textbf{shutdown} procedure (see Section~\ref{sec:shutdown}) so that used
...
typedef struct {
  ...
  /// communication infrastructure build up flag
...
Here is the definition of each of the configuration structure fields:
...
There is a hardware/implementation dependent maximum.
\item[notify\_flag\_num] the desired number of intenal notification flag buffers for weak synchronization to be created. 
There is a hardware/implementation dependent maximum.
...
\emph{Fortran and C differences}
...
Invoking \gaspifunction{config\_get} in any other execution phase than the
setup phase yields undefined behavior.
...
Invoking \gaspifunction{config\_set} in any other execution phase than the
setup phase yields undefined behavior.
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{in}{configuration}{the configuration object with the desired
...
not specified in the standard. This is implementation dependent.
...
variables, a command line argument, a daemon or something completely different.
The actual transfer of knowledge is implementation dependent.

\gaspifunction{proc\_init} registers a given process at the other remote \GASPI{} processes.
If the parameter \parameter{build\_infrastructure} of the passed configuration structure is set,
...
\GASPI{} process running on the second host in the list has rank one and so on and so forth.
...
by passing the list of machines in which the failed node is substituted by the new host.
The newly \GASPI{} process has than the rank of the \GASPI{} process which
has been running on the failed node.

In case of subsequent start of additional \GASPI{} processes, the newly started
\GASPI{} processes register at the other remote \GASPI{} processes.
...
is set, return value \GASPISUCC{} also implies thatt the infrastructure is up and ready to be used.
...
operation can be used already. Connections might be needed to be established.
A segment has to be allocated for passive communication
capabilites. If one sided communication is supposed to be used, than the segment has to
...
% % are corrupted.
%
% Invoking \gaspifunction{substitute\_node} in any other execution phase
% than the working phase yields undefined behavior.
...
and performs the required clean\-up. There is no definition on the
standard on a verification of a healthy global state (i.\,e.\ all
...
invoking \GASPI{} process have been released. That means in specific,
...
used in order to the registration of a user defined signal handler
function which ensures the controlled shut down of an entire \GASPI{} application
on the global level
if the application receives an interrupt signal (\emph{STRG + C}) on the interactive master
...
Invoking \gaspifunction{proc\_kill} in any other execution phase than the
working phase yields undefined behavior.

\begin{useradvice}
The kill signal kills a \GASPI{} process in an uncontrolled way.
...
\subsection{Process management utilities}
...
The total number of started \GASPI{} processes $P$ (c.f. \secref{sec::getrank})
...
If successful, the return value ist \GASPISUCC{} and
...
In case of error, the return value is \GASPIGERR{} and the value of
the \parameter{rank\_num} is undefined.
...
It is bound to the working phase of the \GASPI{} lifecycle.
...
In case of successfull procedure completion, i.e. return value \GASPISUCC,
...
communication between the two nodes. In case that the the connection has been established already,
...
In case of return value \GASPIGERR, the communication infratstructure could not
be established between the local \GASPI{} process and the remote \GASPI{} process.
...
Invoking \gaspifunction{connect} in any other execution
phase than the working phase yields undefined behaviour.
...
In case of successfull procedure completion, i.e. return value \GASPISUCC,
the communication infrastructure is deleted. Associated resources are freed.
In case that the the connection has been deleted already,
e.g. by the connection partner, the return value is \GASPISUCC{}.
...
In case of return value \GASPIGERR, the connection could not be diconnected. 

In case of the latter two return values local resources are freed and a check of the state vector
by invocation of \gaspifunction{state\_vec\_get} gives information
whether the remote \GASPI{} process is still healthy.

Invoking \gaspifunction{disconnect} in any other execution
phase than the working phase yields undefined behaviour.

After successful procedure completion, i.\,e.\ return value \GASPISUCC{},
the connection is deleted and can no longer be used.

In case of error, the return value is \GASPIGERR{}.

Invoking \gaspifunction{disconnect} in any other execution phase than the
working phase yields undefined behavior.
...
A necessary pre-condition for failure tolerant code is a profound
knowledge about the state of the communication partners of the local
\GASPI{} process.
...
There are procedures to inquire the state of the communication partners
...
\item global atomic counters
  \begin{itemize}
    \item \gaspifunction{counter\_reset}
...
Invoking
\gaspifunction{state\_vec\_get} in any other execution phase than the working phase
...
Start-up of mixed MPI and \GASPI{} code is achieved by invoking
...
\GASPI{} and MPI communication should not be at the same time, i.\,e.\
...
sub group. In order to be used, also \GASPIGROUPALL{} has to be committed
by \gaspifunction{group\_commit} if it is needed.
\end{useradvice}
...
Invoking \gaspifunction{group\_create} in any other execution phase than the
working phase yields undefined behavior.
...
a given rank to an empty group.
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{inout}{group}{the group to add the rank to}
...
Invoking
\gaspifunction{group\_add} in any other execution phase than the working phase
yields undefined behavior.
...
\begin{useradvice}
A specific group commit should be performed only by a single thread.
...
Invoking \gaspifunction{group\_commit} in any other execution phase than the working phase
yields undefined behavior.
...
After successful procedure completion, i.\,e.\ return value \GASPISUCC{},
\parameter{group} is deleted and cannot be used.

In case of error, the return value is \GASPIGERR{}.

Invoking \gaspifunction{group\_delete} in any other execution phase than the
working phase yields undefined behavior.
...
After successful procedure completion, i.\,e.\ return value \GASPISUCC{},
\parameter{group\_max} contains the maximum number of allowed groups provided
by \GASPI{}. The value of \parameter{group\_max} is related to the parameter
\parameter{group\_num} in the configuration structure which is retrieved to \gaspifunction{proc\_init} at
startup. If the value of \parameter{group\_num} is compliant with the system capabilities,
\parameter{group\_max} is equal to \parameter{group\_num}. Otherwise, \parameter{group\_max}
is equal to the maximum compliant with the system capabilities. This value might be implementation
specific.

In case of error, the return value is \GASPIGERR{}. The parameter
\parameter{group\_max} has an undefined value.

Invoking
\gaspifunction{group\_max} in any other execution phase than the working phase
yields undefined behavior.
...
Invoking
\gaspifunction{group\_size} in any other execution phase than the working phase
yields undefined behavior.
...
to be unique on the local \GASPI{} process. Creating a new segment
...
After successful procedure completion, i.\,e.\ return value \GASPISUCC{},
the segment can be locally accessed.
...
Invoking \gaspifunction{segment\_alloc} in any other execution
phase than the working phase yields undefined behavior.
...
Invoking \gaspifunction{segment\_register} in any other execution
phase than the working phase yields undefined behavior.
...
After successful procedure completion, i.\,e.\ \GASPISUCC{}, the segment can be locally accessed
...
Invoking
\gaspifunction{segment\_create} in any other execution phase than the working
phase yields undefined behavior.
...
\subsubsection{\gaspifunction{segment\_free}}

The \gaspisemantic{synchronous} \gaspisemantic{local}
\gaspisemantic{blocking} procedure \gaspifunction{segment\_free}
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{in}{segment\_id}{ The segment ID to be freed. }
\parameterlistend
\executionphasework
\FStdRetDescNOTimeout
\end{FDef}

\gaspifunction{segment\_free} releases the resources of the segment
which is referenced by the \parameter{segment\_id} identifier.

After successful procedure completion, i.\,e.\ return value \GASPISUCC{},
the segment is freed and the resources are released. It is a failure to use the segment for communication
between two \GASPI{} processes after \gaspifunction{free} has been called.
...
Invoking \gaspifunction{segment\_free} in any other execution
phase than the working phase yields undefined behavior.
...
%
%
% \gaspifunction{segment\_delete} frees and deregister the segment that is referenced by
...
%
% Invoking
% \gaspifunction{segment\_delete} in any other execution phase than the working
% phase yields undefined behavior.
...
The \gaspifunction{segment\_max} procedure
is a \gaspisemantic{synchronous} \gaspisemantic{local}
\gaspisemantic{blocking} procedure which returns
...
\end{FDefSign}
\parameterlistbegin
  \parameterlistitem{out}{segment\_max}{the total number of allowed segments}
\parameterlistend
\executionphasework
\FStdRetDescNOTimeout
\end{FDef}
...
After successful procedure completion, i.\,e.\ return value \GASPISUCC{},
\parameter{segment\_max} contains the maximum number of allowed segments provided
by \GASPI{}. The value of \parameter{segment\_max} is related to the parameter
\parameter{segment\_num} in the configuration structure which is retrieved to \gaspifunction{proc\_init} at
startup. If the value of \parameter{segment\_num} is compliant with the system capabilities,
\parameter{segment\_max} is equal to \parameter{segment\_num}. Otherwise, \parameter{segment\_max}
is equal to the maximum compliant with the system capabilities. This value might be implementation
specific.

In case of error, the return value is \GASPIGERR{}. The parameter
\parameter{segment\_max} has an undefined value.
...
specified by the triple consisting of the rank, segment identifier and
the offset.  This prevents a global all to all distribution of memory
addresses, since memory addresses of memory segments could be and
normally are different on different \GASPI{} processes,

The write operations have four different variants that allow different
communication patterns:

\begin{itemize}
\item \gaspifunction{write}
\item \gaspifunction{write\_notfiy}
...
The read operations do not support notification calls. This is due to the fact
that a notification can only be sent after ensuring that the communication request
has been processed. This would imply that a subsequent wait call has to be invoked
directly after invoking read. However, this can be managed more effectively by the
application.
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{in}{segment\_id\_local}{the local segment ID to be written from}
\parameterlistitem{in}{offset\_local}{the local offset in bytes to be written from }
\parameterlistitem{in}{rank}{the remote rank to be written to }
\parameterlistitem{in}{segment\_id\_remote}{the remote segment to be written to }
\parameterlistitem{in}{offset\_remote}{the remote offset to be written to}
\parameterlistitem{in}{size}{the size of the message to be sent}
\parameterlistitem{in}{queue}{the queue to be used}
\parameterlistitem{in}{timeout}{the timeout}
\parameterlistend
\executionphasework
\FStdRetDesc
...
of \parameter{size} bytes from a source buffer of the local \GASPI{} process to a target buffer of a
remote \GASPI{} process. This communication request is posted to the communication queue \parameter{queue}.
The source buffer is specified by the pair \parameter{segment\_id\_local}, \parameter{offset\_local}. The
target buffer is specified by the triple \parameter{rank}, \parameter{segment\_id\_remote},
\parameter{offset\_remote}.
% The source buffer is located at address \emph{source\_buffer = segment\_base\_addr + offset\_local},
...
% The target buffer is located at address \emph{target\_buffer = segment\_base + offset\_remote} where
...
been posted to the underlying network infrastructure. The remaining queue depth is decreased
by one.

Successive \gaspifunction{write} calls posted to the same queue and the same destination
rank are non-overtaking. Non-overtaking means that the order of the messages is preserved
on the remote side. In particular, one can assume, that if the last message has arrived on
the remote process, also the previously posted messages have arrived on the remote side.

\gaspifunction{write} calls can be posted from every thread of the \GASPI{} process.
...
on the same queue. A subsequent call of \gaspifunction{write} has to be
...
posted communication requests reaches the queue depth of a given queue, the communication
...
\begin{useradvice}
Return value \GASPISUCC{} does not mean, that the data has been sent or that the data has even
arrived at the remote side. This implies, that writing data into the source buffer along the
communication results in an undefined behavior. Write access to the data in
the source buffer is not allowed until the request has been
processed which is ensured by invoking \gaspifunction{wait}. In contrast to that, it is allowed to read data from
the source buffer even along the communication.
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{in}{segment\_id\_local}{the local segment ID to be read to}
\parameterlistitem{in}{offset\_local}{the local offset in bytes to be read to}
\parameterlistitem{in}{rank}{the remote rank to be read from}
\parameterlistitem{in}{segment\_id\_remote}{the remote segment to be read from}
\parameterlistitem{in}{offset\_remote}{the remote offset to be read from}
\parameterlistitem{in}{size}{the size of the message to be read}
\parameterlistitem{in}{queue}{the queue to be used}
\parameterlistitem{in}{timeout}{the timeout}
\parameterlistend
\executionphasework
...
of \parameter{size} bytes from a source buffer of a remote \GASPI{} process to a target buffer of the
local \GASPI{} process. This communication request is posted to the communication queue \parameter{queue}.
The target buffer is specified by the pair \parameter{segment\_id\_local}, \parameter{offset\_local}. The
source buffer is specified by the triple \parameter{rank}, \parameter{segment\_id\_remote},
\parameter{offset\_remote}.
% The target buffer is located at address \emph{target\_buffer = segment\_base\_addr + offset\_local},
% where \emph{segment\_base\_addr} is the base address of the segment with the identifier \parameter{segment\_id\_local}.
% It is the address which is obtained by applying \gaspifunction{segment\_ptr}.
% The source buffer is located at address \emph{source\_buffer = segment\_base + offset\_remote} where
% \emph{segment\_base} is the base address of the segment represented by \parameter{segment\_id\_remote}
% on the remote \GASPI{} process.
...
Successive \gaspifunction{read} calls posted to the same queue and the same destination
rank are non-overtaking. Non-overtaking means that the order of the messages is preserved
on the local side. In particular, one can assume, that if the last message has arrived on
the local process, also the previously posted messages have arrived on the remote side.

\gaspifunction{read} calls can be posted from every thread of the \GASPI{} process.

If the procedure returns with \GASPITIME{}, the communication request could not be posted
to the hardware during the given timeout. This can happen, if another thread is in a \gaspifunction{wait}
on the same queue. A subsequent call of \gaspifunction{read} has to be
...
\begin{useradvice}
Return value \GASPISUCC{} does not mean, that the data transfer has started or that the data has been even
received at the local side. This implies, that data accesses to the local target buffer result in
an undefined behavior along the data transfer.  Access to the target
buffer is not allowed until the request has been
processed which is ensured by invoking \gaspifunction{wait}.
Use \gaspifunction{notify} to synchronize the communication.
\end{useradvice}
...
After successful procedure completion, i.\,e.\ return value \GASPISUCC{}, the thitherto posted communication
...
\gaspifunction{wait} procedure calls can be posted from every thread of the local \GASPI{} process.
However, the wait operation is a thread exclusive operation and therefore needs privileged
access to the queue which means that if a write/read is done while a
wait is in operation, the write/read operation blocks to ensure correctness. Enforcing this makes it correct and
safer to the user while easier and for the implementor and still
allows for high performance implementation.
That means that successive \gaspifunction{wait} calls invoked on the same queue by different threads
are processed one by another.
...
on the same queue. A subsequent call of \gaspifunction{wait} has to be
...
In both cases, \GASPITIME{} and \GASPIGERR{}, the \GASPI{} state vector
should be checked in order to eliminate the possibility of a failure.
If a failure is detected, all of the communication requests which have been
posted to the given queue since the last \gaspifunction{wait} are in an undefined state.

\begin{useradvice}
  Return value \GASPISUCC{} means, that the data of all posted write
  requests has been sent to the remote side. It does not mean, that
  the sent data has arrived at the remote side. However, write
  accesses to the local source buffer are allowed again.
\end{useradvice}

\begin{useradvice}
  Return value \GASPISUCC{} means, that the data of all posted read
  requests has been arrived at the local side. It is safe to read data
  from the local target buffer.
\end{useradvice}
...
local initialization in order to be sure to read correct data. On the
...
the data has been sent by updating a flag on the remote
side. These notification flags are supposed to be attached
...
flags. This is not thread save. There is a thread safe atomic function
to reset the local flag with a given ID which returns the value 
of the flag before it is reset. 
...
\parameterlistitem{in}{flag\_id}{the remote flag ID where the flag value is sent to}
\parameterlistitem{in}{flag\_val}{the flag value to be sent}
\parameterlistitem{in}{queue}{the queue to be used}
\parameterlistitem{in}{timeout}{the timeout}
\parameterlistend
\executionphasework
...
\parameter{flag} of the local \GASPI{} process to an internal notification flag buffer of a
remote \GASPI{} process. This notification request is posted to the communication queue \parameter{queue}.
The remote notification flag buffer is specified by the pair \parameter{rank}, \parameter{flag\_id}.
% The remote flag buffer is located at address \emph{flag\_buffer = segment\_base + offset\_flag} where
% \emph{segment\_base} is the base address of the segment represented by \parameter{segment\_id\_flag}.
...
Successive \gaspifunction{write} and \gaspifunction{notify} calls posted to the same queue and the same destination
rank are non-overtaking. Non-overtaking means that the order of the messages is preserved
on the remote side. In particular, one can assume, that if the notification message has arrived on
the remote process, also the previously posted messages containing the work load has arrived on the remote side.

\gaspifunction{notify} calls can be posted from every thread of the \GASPI{} process.

If the procedure returns with \GASPITIME{}, the notification request could not be posted
to the hardware during the given timeout. This can happen, if another thread is in a \gaspifunction{wait}
on the same queue. A subsequent call of \gaspifunction{notify} has to be
invoked in order to complete the call.
...
\begin{useradvice}
Return value \GASPISUCC{} does not mean, that the notification has been sent or that the notification
flag has even arrived at the remote side.
\end{useradvice}

\begin{implementoradvice}
\gaspifunction{notify} is semantically equivalent to an update of a local internally managed flag buffer
...
% \end{FDefSign}
% \parameterlistbegin
% \parameterlistitem{in}{segment\_id}{the local segment ID where the flag resides}
% \parameterlistitem{in}{offset}{the local offset where the flag resides}
% \parameterlistitem{in}{timeout}{the timeout}
% \parameterlistend
% \executionphasework
% \FStdRetDesc
...
% non-local procedure which waits until a flag which resides in the local
% partition of the global address space has a value that is not zero.
% 
% The flag buffer on the local side is specified by the
...
% \GASPISUCC{}, the value of the flag buffer has changed to a value that
% is not zero. All threads that are waiting for a single flag are
% notified.
% 
% If the procedure returns with \GASPITIME{}, the flag has not
...
% 
% \begin{useradvice}
%   One standard scenario for the usage of \gaspifunction{notify\_wait}
...
%   the same queue and the same destination rank. Since the standard
%   preserves the order of the messages on the remote side, one can
%   assume, that if the notification flag has arrived on the remote
%   process, also the previously posted messages carrying the work load
%   have arrived.
% \end{useradvice}
% 
% \begin{useradvice}
%   If in a multi-threaded application more than one thread calls
%   \gaspifunction{notify\_wait} for the same flag, then all waiting
...
and \gaspifunction{write\_notify}, \gaspifunction{write\_list\_notify}
...
% out of a number of flags. This could be achieved by busy iterating
% over the list of flags and call \gaspifunction{notify\_wait} with the
...
gaspi_notify_waitsome ( gaspi_flag_id_t flag_id_beg
                      , gaspi_flag_id_t flag_id_num
                      , gaspi_timeout_t timeout
                      )
\end{verbatim}
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{in}{flag\_id\_beg}{the local flag ID for the first flag to wait for}
\parameterlistitem{in}{flag\_id\_num}{the number of flag ID's to wait for}
\parameterlistitem{in}{timeout}{the timeout}
\parameterlistend
\executionphasework
\FStdRetDesc
...
of consecutive flags residing in the local
internal buffer has a value that is not zero.

The flag buffer is specified by the
pair \parameter{flag\_id\_beg}, \parameter{flag\_id\_num}. 
It contains $\mbox{\it flag\_id\_num}$ many consecutive
flags beginning at the flag with ID $\mbox{\it flag\_id\_beg}$.
% The first
% flag is located at the address \emph{segment\_base\_addr +
...
After successfull procedure completion, i.\,e.\ return value
\GASPISUCC{}, the value of at least one of the flags in the flag
buffer has changed to a value that is not zero. All threads that are
waiting for the flags are notified.

If the procedure returns with \GASPITIME{}, no flag has changed during
the given period of time.

In case of an error, i.\,e.\ \GASPIGERR{}, the values of the flags are
undefined.

\begin{useradvice}
  One standard scenario for the usage of \gaspifunction{notify\_waitsome}
  inspecting only one flag
  is the following: The remote side uses a \gaspifunction{write} call
  followed by a subsequent call of \gaspifunction{notify} posted to
  the same queue and the same destination rank. Since the standard
  preserves the order of the messages on the remote side, one can
...
\begin{useradvice}
  If in a multi-threaded application more than one thread calls
  \gaspifunction{notify\_waitsome} for the range of flags, then all waiting
  threads are notified about the change of at least one of the flags. By
  inspecting the actual values of each of the flags with
  \gaspifunction{notify\_reset}, only one thread per changed flag receives a value
  different from zero.
\end{useradvice}
...
\begin{useradvice}
  ...
  of a single flag. The code waits in a blocking manner and thus
  cannot be used in fault tolerant applications.
\end{useradvice}

\insertlisting{waitsome_select.c}{waitsome in a multi-threaded application\label{lst:waitsome_select.c}}
...
% \insertlisting{notify_wait.c}{Generic implementation of \gaspifunction{notify\_wait}.\label{lst:notify_wait.impl}
% waiting for a single flag.}

%%%%%%%%%%%%% reset flag

\subsubsection{\gaspifunction{notify\_reset}}

For the \gaspifunction{notify\_waitsome} procedure, there is a flag
initialization procedure which resets the given flag to zero. It is a
...
gaspi_notify_reset ( gaspi_flag_id_t flag_id
                   , gaspi_flag_t old_flag_val
                   )
\end{verbatim}
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{in}{flag\_id}{the local flag ID to reset}
\parameterlistitem{out}{old\_flag\_val}{flag value before reset}
\parameterlistend
\executionphasework
\FStdRetDescNOTimeout
\end{FDef}

\gaspifunction{notify\_reset} resets the flag with ID \parameter{flag\_id} to
...
The flag buffer on the local side is specified by the flag ID \parameter{flag\_id}.
% It is located at address \emph{flag\_buffer = segment\_base\_addr + offset},
% where \emph{segment\_base\_addr} is the base address of the segment with the identifier \parameter{segment\_id}.
% It is the address which is obtained by applying \gaspifunction{segment\_ptr}.

After successful procedure completion, i.\,e.\ return value
\GASPISUCC{}, the value of the flag buffer was set to zero and
the \parameter{old\_flag\_val} contains the content of the flag buffer
before it was set to zero. To read the old value and to set to value
to zero is a single atomic operation.

\gaspifunction{notify\_waitsome} calls can be posted from every thread of
the \GASPI{} process.
...
\subsubsection{\gaspifunction{notify\_flag\_num}}

The \gaspifunction{notify\_flag\_num} procedure
is a \gaspisemantic{synchronous}
\gaspisemantic{local} \gaspisemantic{blocking}
procedure which determines the number
of available notification flags.
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{out}{flag\_max}{the maximum number of available notification flags}
\parameterlistend
\executionphasework
\FStdRetDescNOTimeout
\end{FDef}
...
After successful procedure completion, i.\,e.\ return value \GASPISUCC{},
\parameter{flag\_max} contains the maximum number of allowed notification flags provided
by \GASPI{}. The value of \parameter{flag\_max} is related to the parameter
\parameter{notify\_flag\_num} in the configuration structure which is retrieved to \gaspifunction{proc\_init} at
startup. If the value of \parameter{notify\_flag\_num} is compliant with the system capabilities,
\parameter{flag\_max} is equal to \parameter{notify\_flag\_num}. Otherwise, \parameter{flag\_max}
is equal to the maximum compliant with the system capabilities. This value might be implementation
specific.

In case of error, the return value is \GASPIGERR{}. The parameter
\parameter{flag\_max} has an undefined value.

Invoking
\gaspifunction{notify\_flag\_num} in any other execution phase than the working phase
yields undefined behavior.
...
%          = WAIT_FLAG (flag_offset[n],0,TEST);
%
%        if (err == SUCCESS)
%        {
%          handle data from n;
...
\gaspifunction{write} with a notification flag on the remote
side. This applies to communication patterns that imply tighter
...
\parameterlistitem{in}{offset\_remote}{the remote offset to be written to}
\parameterlistitem{in}{size}{the size of the message to be sent}
\parameterlistitem{in}{flag\_id}{the remote flag ID where the flag is sent to}
\parameterlistitem{in}{flag\_value}{the value of the flag to be sent}
\parameterlistitem{in}{queue}{the queue to be used}
\parameterlistitem{in}{timeout}{the timeout}
\parameterlistend
\executionphasework
...
\begin{implementoradvice}
The procedure is semantically equivalent to a call of \gaspifunction{write}
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{in}{num}{the number of elements to be sent}
\parameterlistitem{in}{segment\_id\_local[num]}{list of local segment ID's to be written from}
\parameterlistitem{in}{offset\_local[num]}{list of local offsets in bytes to be written from }
\parameterlistitem{in}{rank}{the remote rank to be written to }
\parameterlistitem{in}{segment\_id\_remote[num]}{list of remote segments to be written to }
\parameterlistitem{in}{offset\_remote[num]}{list of remote offsets to be written to}
\parameterlistitem{in}{size[num]}{list of sizes of the messages to be sent}
\parameterlistitem{in}{queue}{the queue to be used}
\parameterlistitem{in}{timeout}{the timeout}
\parameterlistend
\executionphasework
...
of \gaspifunction{write} with the given local and remote buffer specification, provided
...
notification flag that the remote receiver can use to ensure that the
...
\parameterlistitem{in}{offset\_remote[num]}{list of remote offsets to be written to}
\parameterlistitem{in}{size[num]}{list of sizes of the messages to be sent}
\parameterlistitem{in}{flag\_id}{the remote flage ID where the flag is sent to}
\parameterlistitem{in}{flag\_value}{the value of the flag to be sent}
\parameterlistitem{in}{queue}{the queue to be used}
\parameterlistitem{in}{timeout}{the timeout}
\parameterlistend
...
\begin{implementoradvice}
The procedure is semantically equivalent to a call of \gaspifunction{write\_list}
and a subsequent call of \gaspifunction{notify}. However, it should be implemented
more efficiently, if supported by the network infrastructure.
\end{implementoradvice}
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{in}{num}{the number of elements to be read}
\parameterlistitem{in}{segment\_id\_local[num]}{list of local segment ID's to be read to}
\parameterlistitem{in}{offset\_local[num]}{list of local offsets in bytes to be read to}
\parameterlistitem{in}{rank}{the remote rank to be read from}
\parameterlistitem{in}{segment\_id\_remote[num]}{list of remote segments to be read from}
\parameterlistitem{in}{offset\_remote[num]}{list of remote offsets to be read from}
\parameterlistitem{in}{size[num]}{list of sizes of the messages to be read}
\parameterlistitem{in}{queue}{the queue to be used}
\parameterlistitem{in}{timeout}{the timeout}
\parameterlistend
\executionphasework
...
Utilities have the purpose of checking the state of communication and
the state of queues.

\subsubsection{\gaspifunction{queue\_num}}

The \gaspifunction{queue\_num} procedure
is a \gaspisemantic{synchronous}
\gaspisemantic{local} \gaspisemantic{blocking}
procedure which determines the number
of available queues.
...
After successful procedure completion, i.\,e.\ return value \GASPISUCC{},
\parameter{queue\_max} contains the maximum number of allowed queues provided
by \GASPI{}. The value of \parameter{queue\_max} is related to the parameter
\parameter{queue\_num} in the configuration structure which is retrieved to \gaspifunction{proc\_init} at
startup. If the value of \parameter{queue\_num} is compliant with the system capabilities,
\parameter{queue\_max} is equal to \parameter{queue\_num}. Otherwise, \parameter{queue\_max}
is equal to the maximum compliant with the system capabilities. This value might be implementation
specific.

In case of error, the return value is \GASPIGERR{}. The parameter
\parameter{queue\_max} has an undefined value.

Invoking
\gaspifunction{queue\_num} in any other execution phase than the working phase
yields undefined behavior.
...
The queue is reset after a call of \gaspifunction{wait}.

In case of error, the return value is \GASPIGERR{}. The parameter \parameter{queue\_size}
has an undefined value.

Invoking
\gaspifunction{queue\_size} in any other execution phase than the working phase yields
undefined behavior.

\subsubsection{\gaspifunction{queue\_size\_max}}

The \gaspifunction{queue\_size\_max} procedure
is a \gaspisemantic{synchronous}
\gaspisemantic{local} \gaspisemantic{blocking} procedure which determines the number
of maximally allowed communication requests per given queue.
...
\parameterlistbegin
\parameterlistitem{in}{queue}{the queue to probe}
\parameterlistitem{out}{queue\_size}{the number of simultaneously allowed requests}
\parameterlistend
\executionphasework
\FStdRetDescNOTimeout
\end{FDef}
...
After successful procedure completion, i.\,e.\ return value \GASPISUCC{},
\parameter{queue\_size} contains the maximum number of allowed simultaneous communication
requests for \parameter{queue} provided
by \GASPI{}. The value of \parameter{queue\_size} is related to the parameter
\parameter{queue\_size\_max} in the configuration structure which is retrieved to \gaspifunction{proc\_init} at
startup. If the value of \parameter{queue\_size\_max} is compliant with the system capabilities,
\parameter{queue\_size} is equal to \parameter{queue\_size\_max}. Otherwise, \parameter{queue\_size}
is equal to the maximum compliant with the system capabilities. This value might be implementation
specific.

In case of error, the return value is \GASPIGERR{}. The parameter
\parameter{queue\_size} has an undefined value.

Invoking
\gaspifunction{queue\_size\_max} in any other execution phase than the working phase
yields undefined behavior.


\subsubsection{\gaspifunction{message\_size\_max}}

The \gaspifunction{message\_size\_max} procedure
is a \gaspisemantic{synchronous}
\gaspisemantic{local} \gaspisemantic{blocking}
procedure which determines the maximally allowed message size for single sided communication.
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{out}{message\_size}{the maximally allowed message size for a single request}
\parameterlistend
\executionphasework
\FStdRetDescNOTimeout
\end{FDef}
...
After successful procedure completion, i.\,e.\ return value \GASPISUCC{},
\parameter{message\_size} contains the maximum message size allowed
for a single one sided communication. The value
of \parameter{message\_size} is related to the parameter
\parameter{message\_size\_max} in the configuration structure which is retrieved to \gaspifunction{proc\_init} at
startup. If the value of \parameter{message\_size\_max} is compliant with the system capabilities,
\parameter{message\_size} is equal to \parameter{message\_size\_max}. Otherwise, \parameter{message\_size}
is equal to the maximum compliant with the system capabilities. This value might be implementation
specific.

In case of error, the return value is \GASPIGERR{}. The parameter
\parameter{message\_size} has an undefined value.

Invoking
\gaspifunction{message\_size\_max} in any other execution phase than the working phase
yields undefined behavior.
...
\begin{todo}
\item The next section is done already in the respective procedure definitions

 \subsection{semantics and correctness}

\item Here, something should be written concerning the validity of
 local memory under the influence of remote read and writes

 \begin{itemize}
 \item Undefined behavior if one reads from or writes into a memory region which is
       the destination of a remote write
 \item Undefined behavior if one writes into a local memory region which is the source
       for a remote read
 \end{itemize}
\end{todo}
...
\begin{itemize}
\item Distributed update
\item Pass arguments and results
\item Atomic operations
\end{itemize}

The implementation should try to enforce fairness in communication
that is, no sender should starve to see its communication request
finished.

The passive keyword means the communication calls should avoid
...
\gaspisemantic{time-based blocking}. Due to the asynchronicity of the
...
called by the sender side to engage in passive communication. To make
sure the requests are processed the \gaspifunction{passive\_wait} is
used. It is an \gaspisemantic{asynchronous}
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{in}{segment\_id\_local}{the local segment ID where the message is send from}
\parameterlistitem{in}{offset\_local}{the local offset where the message is sent from}
\parameterlistitem{in}{rank}{the remote Rank where the message is sent to}
\parameterlistitem{in}{size}{the size of the message to be sent to}
\parameterlistitem{in}{tag}{the tag to be sent with the message}
\parameterlistitem{in}{timeout}{the timeout}
\parameterlistend
\executionphasework
\FStdRetDesc
...
bytes from a source buffer of the local \GASPI{} process to the remote
...
be posted. The source buffer is specified by the
pair \parameter{segment\_id\_local}, \parameter{offset\_local}.
% The
% source buffer is located at address \emph{source\_buffer =
...
There is a size limit for messages send with
\gaspifunction{passive\_send}. The maximum size is returned by the function
\gaspifunction{passive\_message\_size\_max}.

The passive message \parameter{tag} can be used to discriminate different messages.
...
Successive \gaspifunction{passive\_send} calls posted to the same destination
rank are non-overtaking. Non-overtaking means that the order of the messages is preserved
on the remote side. In particular, one can assume, that if the last message has arrived on
the remote process, also the previously posted messages have arrived on the remote side.

\gaspifunction{passive\_send} calls can be posted from every thread of the \GASPI{} process.
...
% If the passive communication queue is full, i.\,e.\ that the number of
% posted communication requests reaches the queue depth, and another communication
...
posted, i.\,e.\ that the number of
posted communication requests reaches the queue depth already, the communication
...
\begin{useradvice}
Return value \GASPISUCC{} does not mean, that the data has been sent or that the data has even
arrived at the remote side. This implies, that write accesses to the source buffer result in
an undefined behavior. In contrast to that, one can still safely access the source buffer by
a read operation. Use \gaspifunction{notify} to synchronize the communication.
\end{useradvice}


\begin{useradvice}
If the parameter \parameter{build\_infrastructure} is not set, a connection has to be established between the processes before the \gaspifunction{passive\_send} can be be used. This is accomplished calling the procedure \gaspifunction{connect}.
\end{useradvice}
...
After successful procedure completion, i.\,e.\ return value \GASPISUCC{}, the thitherto posted
...
\gaspifunction{passive\_wait} procedure calls can be posted from every thread of the local \GASPI{} process.
However, the wait operation is a thread exclusive operation and therefore needs privileged
access to the queue which means that if a passive send is done while a
wait is in operation, the passive send blocks to ensure correctness. Enforcing this makes it correct and
safer to the user while easier and for the implementor and still
allows for high performance implementation.
That means that successive \gaspifunction{passive\_wait} calls invoked by different threads
are processed one by another.

If the procedure returns with \GASPITIME{}, the wait request could not be completed
during the given timeout. This can happen, if there are still some passive communication requests
to be completed or if there is another thread in a \gaspifunction{passive\_wait} on the same queue.

If the procedure returns with \GASPIGERR{}, the wait request aborted abnormally.

In both cases, \GASPITIME{} and \GASPIGERR{}, the \GASPI{} state vector
should be checked in order to eliminate the possibility of a failure.
If a failure is detected, all of the \gaspifunction{passive\_send} requests which have been
posted to the given queue since the last \gaspifunction{passive\_wait} are in an undefined state.

\begin{useradvice}
Return value \GASPISUCC{} means, that the data of all posted passive communication requests has been sent
to the remote side. It does not mean, that the data has arrived at the remote side. This implies, that
write accesses to the local source buffer are possible again.
\end{useradvice}
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{in}{segment\_id\_local}{the local segment ID where the message is written to}
\parameterlistitem{in}{offset\_local}{the local offset where the message is written to}
\parameterlistitem{out}{rank}{the remote rank from which the message is received}
\parameterlistitem{out}{tag}{the tag which has been sent with the message}
\parameterlistitem{in}{timeout}{the timeout}
\parameterlistend
\executionphasework
\FStdRetDesc
...
\gaspifunction{passive\_receive} receives a contiguous block of data into a target buffer from some unspecified
remote \GASPI{} process. The target buffer is specified by the pair \parameter{segment\_id\_local},
...
The \gaspifunction{passive\_receive} procedure is supposed to be posted before a corresponding
\gaspifunction{passive\_send} procedure is invoked.

The target buffer needs to have enough space to hold the maximal passive message size. Otherwise,
the received message might overwrite memory regions outside of the buffer.

A valid \gaspifunction{passive\_receive} communication request requires that the local destination
segment is allocated and that there is a connection between the local
and the remote \GASPI{} process from which a message is coming. Otherwise, the communication request is invalid
and the procedure returns with \GASPIGERR{}.

After successful procedure completion, i.\,e.\ return value \GASPISUCC{}, the data has been received
and is available at the target buffer. Further \parameter{rank} contains the rank of the sending
process and \parameter{tag} contains the tag associated to the message.

Successive \gaspifunction{passive\_receive} calls posted by two disjoint threads using two different
target buffers yield an undefined behavior. Either the first incoming message is received by the
first thread or the second.
That means that the \gaspifunction{passive\_receive} should be posted only from a single thread of
the \GASPI{} process.

If the procedure returns with \GASPITIME{}, there was no pending communication request
in the queue. The output parameters \parameter{rank} and \parameter{tag} contain undefined values.


\begin{useradvice}
As long as there is a pending \gaspifunction{passive\_receive}, reading from or writing into
the correspondent receive buffer, either local or remote,  yields undefined results.
\end{useradvice}

\begin{implementoradvice}
The implementation should try to enforce fairness in communication
that is, no sender should starve to see its communication request
finished. The passive keyword means the communication calls should avoid
busy-waiting and consume no CPU cycles, freeing the system for
computation.
\end{implementoradvice}
...
The \gaspifunction{passive\_queue\_size} procedure
is a \gaspisemantic{synchronous}
\gaspisemantic{local} \gaspisemantic{blocking} procedure which determines the number
of communication requests posted to the passive channel.
...
The queue is reset after a call of \gaspifunction{passive\_wait}.

In case of error, the return value is \GASPIGERR{}. The value of \parameter{queue\_size}
is undefined.

\subsubsection{\gaspifunction{passive\_queue\_size\_max}}

The \gaspifunction{passive\_queue\_size\_max} procedure
is a \gaspisemantic{synchronous}
\gaspisemantic{local} \gaspisemantic{blocking} procedure which determines the number
of maximally allowed simultaneous passive communication requests.
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{out}{queue\_size}{Maximally available number of requests in the passive communication queue}
\parameterlistend
\executionphasework
\FStdRetDescNOTimeout
\end{FDef}
...
After successful procedure completion, i.\,e.\ return value \GASPISUCC{},
\parameter{queue\_size} contains the maximum number of allowed simultaneous passive communication
requests by \GASPI{}. The value of \parameter{queue\_size} is related to the parameter
\parameter{passive\_queue\_size\_max} in the configuration structure which is retrieved to \gaspifunction{proc\_init} at
startup. If the value of \parameter{passive\_queue\_size\_max} is compliant with the system capabilities,
\parameter{queue\_size} is equal to \parameter{passive\_queue\_size\_max}. Otherwise, \parameter{queue\_size}
is equal to the maximum compliant with the system capabilities. This value might be implementation
specific.
...
Invoking
\gaspifunction{passive\_queue\_size\_max} in any other execution phase than the working phase
yields undefined behavior.


\subsubsection{\gaspifunction{passive\_message\_size\_max}}

The \gaspifunction{passive\_message\_size\_max} procedure
is a \gaspisemantic{synchronous}
\gaspisemantic{local} \gaspisemantic{blocking} procedure which determines the
maximally allowed message size which can be posted to the passive channel.
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{out}{message\_size}{maximal message size per single passive communication request}
\parameterlistend
\executionphasework
\FStdRetDescNOTimeout
\end{FDef}
...
After successful procedure completion, i.\,e.\ return value \GASPISUCC{},
\parameter{message\_size} contains the maximum message size for passive communication.
The value of \parameter{message\_size} is related to the parameter
\parameter{passive\_message\_size\_max} in the configuration structure which is retrieved to \gaspifunction{proc\_init} at
startup. If the value of \parameter{passive\_message\_size\_max} is compliant with the system capabilities,
\parameter{message\_size} is equal to \parameter{passive\_message\_size\_max}. Otherwise, \parameter{message\_size}
is equal to the maximum compliant with the system capabilities. This value might be implementation
specific.
...
Invoking
\gaspifunction{passive\_message\_size\_max} in any other execution phase than the working phase
yields undefined behavior.
...
Atomic counters are globally accessible variables to which the access
is atomic through two operations: fetchAdd and cmpSwap.
...
process should starve to see its atomic operation request
finished.
...
\begin{implementoradvice}
  ...
  available nodes to avoid to much contention on a single node holding
  the atomic counters.
\end{implementoradvice}

\subsection{Atomic operation calls}

\subsubsection{\gaspifunction{counter\_reset}}

The \gaspifunction{counter\_reset} procedure is a \gaspisemantic{synchronous}
\gaspisemantic{non-local} \gaspisemantic{blocking} procedure
which atomically resets a global counter.
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{in}{counter\_id}{the counter to be reseted}
\parameterlistitem{in}{value}{the value to which the global atomic counter is reseted}
\parameterlistitem{in}{timeout}{the timeout}
\parameterlistend
\executionphasework
\FStdRetDesc
...
\gaspifunction{counter\_resets} atomically resets the value of the counter \parameter{counter\_id}
to the value given by \parameter{value}.

After successful procedure completion, i.\,e.\ return value \GASPISUCC{}, the value of the atomic counter
is equal to \parameter{value}.

If the procedure returns with \GASPITIME{}, the reset request could not be completed
during the given timeout. A subsequent call of \gaspifunction{counter\_reset} needs
...
Invoking
\gaspifunction{counter\_reset} in any other execution phase than the working phase
yields undefined behavior.
...
Invoking
\gaspifunction{counter\_fetch\_add} in any other execution phase than the working phase
yields undefined behavior.
...
\parameterlistitem{in}{value\_new}{the new value to which the counter is set if the comparison is true}
\parameterlistitem{out}{value\_old}{the value of the counter before the operation}
\parameterlistitem{in}{timeout}{the timeout}
\parameterlistend
\executionphasework
...
\subsection{Atomic operation utilities}

\subsubsection{\gaspifunction{counter\_num}}

The \gaspifunction{counter\_num} procedure is a \gaspisemantic{synchronous}
\gaspisemantic{local} \gaspisemantic{blocking} procedure
which returns the total number of allowed atomic counters.
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{out}{counter\_max}{The number of available global atomic counters}
\parameterlistend
\executionphasework
\FStdRetDescNOTimeout
\end{FDef}
...
After successful procedure completion, i.\,e.\ return value \GASPISUCC{},
\parameter{counter\_max} contains the maximum number of allowed global atomic counters.
The value of \parameter{counter\_max} is related to the parameter
\parameter{counter\_num} in the configuration structure which is retrieved to \gaspifunction{proc\_init} at
startup. If the value of \parameter{counter\_num} is compliant with the system capabilities,
\parameter{counter\_max} is equal to \parameter{counter\_num}. Otherwise, \parameter{counter\_max}
is equal to the maximum compliant with the system capabilities. This value might be implementation
specific.

In case of error, the return value is \GASPIGERR{}. The parameter
\parameter{counter\_max} has an undefined value.

Invoking
\gaspifunction{counter\_num} in any other execution phase than the working phase
yields undefined behavior.
...
Successful collective procedure completion is achieved only if all
...
well as time-based blocking. That means, progression towards successful procedure
...
operation of a specific type on a given group can run at a time. Starting a
...
Two allreduce for one group can not run at the same time. However, for example
an allreduce and a barrier can run at the same time.

The timeout is a necessary condition in order to be able to write
failure tolerant code. Timeout $=0$ makes progress in the operation
if possible. However, the procedure returns as soon as the progress
is achieved.
...
\begin{useradvice}
Not every collective operation will be implementable in an asynchronous fashion - for 
example if a user-defined callback function is used within a global reduction. Progress in this case
 can only be achieved inside of the call. Especially for large systems this implies that a collective
potentially has to be called a substantial number of times in order to complete -- especially if used in combination with \GASPITEST{}. In this combination the called collective immediately returns (after completing local work) and never waits for messages from remote processes. A corresponding code fragment in this case would assume the form: 
...
Progression towards successful \gaspifunction{barrier} completion is achieved even if the procedure
...
barrier operation in another thread before another one is not finished
...
and only then a full synchronization is applied by calling again the
barrier with a blocking semantics (if needed).
...
Progression towards successful \gaspifunction{allreduce} completion is achieved even if the procedure
...
operation on a given group can run at a time. Starting a
reduction operation on the same group in a separate thread before another one
is not finished on all processes of the group
is not allowed and yields undefined behavior.
...
an internal buffer into the receive buffer right before the procedure returns
successfully.
...
undefined behavior.
The maximally allowed number of elements
...
% Two simultaneous collective operations based on the same group are not allowed
...
The \gaspifunction{allreduce\_elem\_max} procedure is a \gaspisemantic{synchronous}
\gaspisemantic{local} \gaspisemantic{blocking} procedure
which returns the total number of allowed elements used in \gaspifunction{allreduce}.
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{out}{elem\_max}{the maximum number of allowed elements in \gaspifunction{allreduce}}
\parameterlistend
\executionphasework
\FStdRetDescNOTimeout
\end{FDef}
...
After successful procedure completion, i.\,e.\ return value \GASPISUCC{}, the parameter
\parameter{elem\_max} contains the maximal allowed number of elements for \gaspifunction{allreduce}.

In case of error, the return value is \GASPIGERR{}. The parameter \parameter{elem\_max}
has an undefined value.

Invoking
\gaspifunction{elem\_max} in any other execution phase than the working phase yields undefined
behavior.
...
Invoking
\gaspifunction{allreduce\_user} in any other execution phase than the working phase
yields undefined behavior.
...
The \gaspifunction{allreduce\_buf\_size} procedure is a \gaspisemantic{synchronous}
\gaspisemantic{local} \gaspisemantic{blocking} procedure
which returns the size of the internal buffer provided by \gaspifunction{allreduce\_user}.
...
After successful procedure completion, i.\,e.\ return value \GASPISUCC{}, the parameter
\parameter{buf\_size} contains the size of the internal buffer of \gaspifunction{allreduce\_user}.

In case of error, the return value is \GASPIGERR{}. The parameter \parameter{buf\_size}
has an undefined value.

Invoking
\gaspifunction{allreduce\_buf\_size} in any other execution phase than the working phase
yields undefined behavior.
...
Invoking
\gaspifunction{time\_get} in any other execution phase than the working phase
yields undefined behavior.
...
Invoking
\gaspifunction{time\_ticks} in any other execution phase than the working phase
yields undefined behavior.
