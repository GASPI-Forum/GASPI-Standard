...
%completed if a corresponding wait operation has returned successfully.
%In this respect, an entire communication request is split into two
...
queues. These queues facilitate higher scalability and can be used as
...
different queues, i.\,e.\ no queue should have its
communication requests delayed indefinitely.

Furthermore, the order of messages addressing the same remote \GASPI{}
process posted to a given queue is preserved, both on the local as
...
causing corruption (c.f. Section 10).  There are two basic operations on atomic
...
The standard guarantees fairness, i.\,e.\ no process should have 
...
run at a time. For example, two allreduce operations for one group can not run at the same
time; however, an allreduce and a barrier can run at the
same time.
...
\TDefDesc{The \GASPI{} Tag type. Tag are used to discriminate between
...
\item \textbf{Initialization}
  \begin{description}
%   \item Initialization of the communication infrastructure
...
resources and communication infrastructure used is cleaned up.
...
\begin{description}
\item[group\_max] the desired maximum permissible number of groups. There is a hardware/implementation dependent maximum.
\item[segment\_max] the desired maximum permissible number of segments per \GASPI{} process.
...
dependent maximum.
\item[queue\_size\_max] the desired number of simultaneously allowed on-going requests in a one-sided communication queue.
There is a hardware/implementation dependent maximum.
...
There is a hardware/implementation dependent maximum.
\item[passive\_queue\_size\_max] the desired number of simultaneously allowed on-going requests in the
...
variables, a command line argument, a daemon or something other mechanism.
The actual transfer of knowledge is implementation dependent.

\gaspifunction{proc\_init} registers a given process with the other remote \GASPI{} processes
and sets the corresponding entry in the state vector to a healthy state. 
...
\GASPI{} process running on the second host in the list has rank one etc..
...
\GASPI{} processes register with the other remote \GASPI{} processes.
...
of  \GASPI{} processes started. Each process has a unique rank associated with it.
...
standard of a verification of a healthy global state (i.\,e.\ all
...
\begin{useradvice}
If the parameter \parameter{build\_infrastructure} is not set, a connection has to be established between the processes before the segment can be registered at the remote process. This is accomplished by calling the procedure \gaspifunction{connect}.
\end{useradvice}
...
After successful procedure completion, i.\,e.\ return value \GASPISUCC{},
the segment is deleted and the resources are released. It would be an application error to use the segment for communication
...
been posted to the underlying network infrastructure. The remaining queue size is decreased
by one.
...
the remote process, the the previously posted messages have also arrived on the remote side.
...
arrived at the remote side. This implies, that writing data into the source location during the
communication results in an undefined behavior. Write access to the data in
the source location is not allowed until the request has been
processed which is ensured by invoking \gaspifunction{wait}. In contrast to that, it is allowed to read data from
the source location even during the communication.
...
\parameter{offset\_remote}.
% The target location is located at address \emph{target\_location = segment\_base\_addr + offset\_local},
% where \emph{segment\_base\_addr} is the base address of the segment with the identifier \parameter{segment\_id\_local}.
% It is the address which is obtained by applying \gaspifunction{segment\_ptr}.
% The source location is located at address \emph{source\_location = segment\_base + offset\_remote} where
% \emph{segment\_base} is the base address of the segment represented by \parameter{segment\_id\_remote}
% on the remote \GASPI{} process.
...
the local process, then the previously posted messages have also arrived on the remote side.
...
received at the local side. This implies, that data accesses to the local target location result in
an undefined behavior during the data transfer.  Access to the target
location is not allowed until the request has been
...
wait is in operation, the write/read operation blocks to ensure correctness. Enforcing this provides correctness and safety to the user while being easier  for the implementor and still
allows fora  high performance implementation.
As a conseuqence, successive \gaspifunction{wait} calls invoked on the same queue by different threads
...
posted to the given queue since the last \gaspifunction{wait} are in an undefined state. Here,
undefined state means that the local \GASPI{} process does not know which requests
have been processed and which requests are still outstanding. A call to 
\gaspifunction{queue\_purge} has to be invoked in order to reset the 
queue.
...
\parameterlistitem{in}{notification\_id}{the remote notification ID to which the notification value is sent}
...
the remote process, then the previously posted messages containing the work load have also arrived on the remote side.
...
and \gaspifunction{write\_notify}, \gaspifunction{notify\_waitsome}
...
\begin{useradvice}
  ...
  preserves the order of the messages on the remote side, one can
  assume, that if the notification has arrived on the remote
  process, then the previously posted messages carrying the work load
  have also arrived.
\end{useradvice}
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{in}{num}{the number of blocks of data to be sent}
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{in}{num}{the number of blocks of data to be read}
...
The \gaspifunction{queue\_purge} procedure
is a \gaspisemantic{synchronous}
\gaspisemantic{local} \gaspisemantic{time-based blocking} procedure which purges a given 
queue.
...
Invoking
\gaspifunction{queue\_purge} in any other execution phase than the working phase yields
undefined behavior.
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{in}{segment\_id\_local}{the local segment ID from which the message is sent}
\parameterlistitem{in}{offset\_local}{the local offset from which the message is sent}
\parameterlistitem{in}{rank}{the remote Rank to which the message is sent}
\parameterlistitem{in}{size}{the size of the message to be sent}
...
rank are non-overtaking. Non-overtaking means that the order of the messages is preserved
on the remote side. In particular, one can assume, that if the last message has arrived on
the remote process, then the previously posted messages have also arrived on the remote side.
...
% posted communication requests reaches the queue size, and another communication
...
wait is in operation, the passive send blocks to ensure correctness. Enforcing this provides correctness and safety to the user while being easier  for the implementor and still
allows for a high performance implementation.
...
undefined state means that the local \GASPI{} process does not know which requests
have been processed and which requests are still outstanding. A call to 
\gaspifunction{passive\_queue\_purge} has to be invoked in order to reset the 
passive queue.
...
% It is located at address \emph{source\_location = segment\_base\_addr + offset\_local},
% where \emph{segment\_base\_addr} is the base address of the segment with the identifier \parameter{segment\_id\_local}.
...
The target location needs to have enough space to hold the maximal passive message size. Otherwise,
...
target locations yield an undefined behavior. There is no possibility to specify which thread will receive which message.
...
the correspondent receive location, either local or remote,  yields undefined results.
\end{useradvice}

\begin{implementoradvice}
The implementation should try to enforce fairness in communication
that is, no sender should have its communication request
delayed indefinitely. The passive keyword means the communication calls should avoid
busy-waiting and consume no CPU cycles, freeing the system for
computation.
\end{implementoradvice}
...
is a \gaspisemantic{synchronous}
\gaspisemantic{local} \gaspisemantic{blocking} procedure which determines the number
of communication requests posted to the passive queue.
...
The \gaspifunction{passive\_queue\_purge} procedure
is a \gaspisemantic{synchronous}
\gaspisemantic{local} \gaspisemantic{time-based blocking} procedure which purges the passive 
queue.
...
Invoking
\gaspifunction{passive\_queue\_purge} in any other execution phase than the working phase yields
undefined behavior.
...
\gaspisemantic{non-local} \gaspisemantic{time-based blocking} procedure
which atomically performs a compare-and-swap operation on a global counter.
...
failure tolerant code. \GASPITEST{} (Timeout $=0$) makes an atomic portion 
of progress in the operation if possible. If progress is possible, the procedure returns as 
soon as the atomic portion of progress is achieved. Otherwise, the procedure returns
immediately. Here, an atomic portion of progress is defined as 
the smallest set of non-dividable instructions in the current state of the collective operation. 
...
barrier again with a blocking semantic (if needed).
...
operation on a given group can run at a time. Starting a
reduction operation on the same group in a separate thread beforepreviously issued operation
...
the parameter is set to the requested/preferred parametervalue. Otherwise, the parameter is set to the maximum value compliant 
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{out}{message\_size}{the maximum  message size allowed for a single request}
\parameterlistend
\executionphasework
\FStdRetDescNOTimeout
\end{FDef}
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{out}{notification\_max}{the maximum number of  notifications available}
\parameterlistend
\executionphasework
\FStdRetDescNOTimeout
\end{FDef}
