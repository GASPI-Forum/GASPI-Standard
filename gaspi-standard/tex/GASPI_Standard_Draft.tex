...
\newcommand{\GASPIQUEUETXT}{operation could not be posted due to a full queue}
...
\newcommand{\FStdRetDescQueue}
{
  \returnlistbegin
  \returnlistitem{\GASPISUCC}{\GASPISUCCTXT}
  \returnlistitem{\GASPITIME}{\GASPITIMETXT}
  \returnlistitem{\GASPIGERR}{\GASPIGERRTXT}
  \returnlistitem{\GASPIQUEUE}{\GASPIQUEUETXT}
  \fdeffinal\returnlistend
}
...
%Furthermore, the order of communication requests addressing the same remote \GASPI{}
...
\begin{description}
 ...
 \item[\GASPIGERR{}] implies that the procedure has terminated due to an error.
  ...
  readable format.
\item [\GASPIQUEUE{}]implies that one of the function calls \gaspifunction{notify},
  \gaspifunction{write}, \gaspifunction{write\_notify}, \gaspifunction{write\_list\_notify},
  \gaspifunction{read}, \gaspifunction{read\_notify} and \gaspifunction{read\_list\_notify}
  has reached the end of the queue and that the corresponding communication request
  could not be issued. If \GASPIQUEUE{} is returned, users should either switch
  to another queue or wait (see \gaspifunction{wait}) and subsequently re-issue the communication request.
\end{description}
...
\begin{TDef}
\TDefDef{\gaspitype{notification\_id}}
\TDefDesc{The \GASPI{} notification ID type.}
\end{TDef}

% \begin{TDef}
% \TDefDef{\gaspitype{tag}}
...
\begin{implementoradvice} The sum of the sizes of
  \gaspitype{notification} should be at most 8 bytes in order to allow
  for Infiniband specific optimizations.
\end{implementoradvice}


\begin{TDef}
  \TDefDef{\gaspitype{atomic\_value}} \TDefDesc{The \GASPI{} global
    atomic value type. An atomic value is unsigned and its maximum
    value can be queried using \gaspifunction{atomic\_max}.}
\end{TDef}

\begin{TDef}
\TDefDef{\gaspitype{return}}
\TDefDesc{The \GASPI{} return value type.}
\end{TDef}

\begin{TDef}
\TDefDef{{\tt vector<\gaspitype{return}>}}{\gaspitype{returns}}
...
\begin{TDef}
\TDefDef{\GASPISUCC{}}
\TDefDesc{GASPI\_SUCCESS is returned if a procedure call is completed successfully.}
\end{TDef}


\begin{TDef}
\TDefDef{\GASPITIME{}} 
\TDefDesc{GASPI\_TIMEOUT is returned if a procedure call ran into a timeout.}
\end{TDef}


\begin{TDef}
\TDefDef{\GASPIGERR{}} 
\TDefDesc{GASPI\_ERROR is returned if a procedure call finished with an error.}
\end{TDef}

\begin{TDef}
  \TDefDef{\GASPIQUEUE{}}
  \TDefDesc{GASPI\_QUEUE\_FULL is returned if the end of the used queue has been reached.}
\end{TDef}

\subsubsection{State vector states}
\begin{TDef}
\TDefDef{GASPI\_STATE\_HEALTHY} 
\TDefDesc{GASPI\_STATE\_HEALTHY implies that a remote \GASPI{} process is healthy and communication is possible.}
\end{TDef}

\begin{TDef}
\TDefDef{GASPI\_STATE\_CORRUPT}
\TDefDesc{GASPI\_STATE\_CORRUPT implies that the remote \GASPI{} process is corrupted and communication is impossible.}
\end{TDef}
...
%    \item Creation of segments (optional).
  \end{description}
...
\gaspifunction{config\_set}, the requested values are just proposals.
Depending on the underlying hardware capabilities, the implementation
is allowed to overrule these proposals.  \gaspifunction{config\_set}
...
application before calling the \gaspifunction{proc\_init} procedure.

After successful procedure completion, \gaspifunction{proc\_init}
returns \GASPISUCC{} and it guarantees that the application has been
started on all hosts. In case that
the \parameter{build\_infrastructure} is set, return value
...
\gaspifunction{proc\_num}.  This is a \gaspisemantic{local}
\gaspisemantic{synchronous} \gaspisemantic{blocking} procedure.
...
\gaspifunction{proc\_num} retrieves the total number of processes that
...
\gaspifunction{proc\_kill} sends an interrupt signal to a given
\GASPI{} process.  It is a \gaspisemantic{synchronous}
...
\insertlisting{../examples/hello_world.f90}{\GASPI{} hello world example in f90. \label{lst:hello_world_f90}}
...
by invocation of \gaspifunction{state\_vec\_get} gives information on
...
\gaspifunction{state\_vec\_get} gives information whether the remote
...
\GASPI{} process, which is the \gaspitype{state}
type. \gaspitype{state} can have one of two values:

\begin{description}
 \item[GASPI\_STATE\_HEALTHY] implies that the remote \GASPI{} process is healthy, i.\,e.\
communication is possible.
 \item[GASPI\_STATE\_CORRUPT] means that the remote \GASPI{}
process is corrupted, i.\,e.\ there is no communication possible.
\end{description}

\begin{TDef}
\TDefDefn{{\tt vector<\gaspitype{state}>}}{\gaspitype{state\_vector}}
\TDefDesc{\gaspitype{state\_vector} is a vector with state information for individual processes. The
  length of the vector equals the number of processes in the \GASPI{}
...
initialized during \gaspifunction{proc\_init}. It is updated, case
...
\gaspifunction{state\_vec\_get} retrieves in case of successful
...
The \gaspifunction{group\_create} procedure is a
\gaspisemantic{synchronous} \gaspisemantic{local}
\gaspisemantic{blocking} procedure which creates an empty group.
...
The \gaspifunction{group\_add} procedure is a
\gaspisemantic{synchronous} \gaspisemantic{local}
\gaspisemantic{blocking} procedure which adds a given rank to an
existing group.
...
or \gaspifunction{segment\_create} after the \GASPI{} startup.
...
retrieved by invoking \gaspifunction{segment\_max}. 
...
The \parameter{alloc\_policy} is used to pass an allocation
...
\gaspifunction{segment\_create} is a \gaspisemantic{synchronous}
...
of \gaspifunction{segment\_alloc}, \gaspifunction{segment\_register}
...
all group members beforehand, \gaspifunction{segment\_create} will
...
\gaspifunction{passive\_send} or as a destination segment for
\gaspifunction{passive\_receive}.  Furthermore, it can be used for
one-sided communication requests, which are invoked by the remote
processes forming the group \parameter{group} or global atomic
operations. The segment \parameter{segment\_id} is ready to be used.
...
\gaspifunction{segment\_create} has to be invoked in order to complete
...
\subsubsection{\gaspifunction{segment\_bind}}
The \gaspisemantic{synchronous} \gaspisemantic{local}
\gaspisemantic{blocking} procedure \gaspifunction{segment\_bind} binds
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{in}{segment\_id}{Unique segment ID to bind.}
...
\parameterlistitem{in}{size}{The size of the memory provided by
  \parameter{pointer} in bytes.}
\parameterlistitem{in}{memory\_description}{The description of the
  memory provided.}
\parameterlistend
...
\begin{verbatim}
gaspi_return_t 
gaspi_segment_bind ( gaspi_segment_id_t segment_id
...
\gaspifunction{segment\_bind} binds the segment identified by the
identifier \parameter{segment\_id} to the user provided memory of size
\parameter{size} located at the address \parameter{pointer}. To
provide less than \parameter{size} bytes results in undefined
behavior. The identifier \parameter{segment\_id} must be unique in the
...
be retrieved by invoking \gaspifunction{segment\_max}.
...
\subsubsection{\gaspifunction{segment\_use}}
The \gaspisemantic{synchronous} \gaspisemantic{collective}
\gaspisemantic{time-based} \gaspisemantic{blocking} procedure
\gaspifunction{segment\_use} is semantically equivalent to a
collective aggregation of \gaspifunction{segment\_bind},
\gaspifunction{segment\_register} and \gaspifunction{barrier}
...
\gaspifunction{segment\_use} will accomplish this as well.
...
\begin{verbatim}
gaspi_return_t 
gaspi_segment_use ( gaspi_segment_id_t segment_id
                  , gaspi_pointer_t pointer
                  , gaspi_size_t size
                  , gaspi_group_t group
                  , gaspi_timeout_t timeout
...
\gaspifunction{segment\_use} binds the segment identified by the
identifier \parameter{segment\_id} to the user provided memory of size
\parameter{size} located at the address \parameter{pointer}. To
provide a \parameter{size} larger than the actual buffer size pointed
by \parameter{pointer} results in undefined
behavior. \gaspifunction{segment\_use} makes the segment referenced by
the
\parameter{segment\_id} identifier visible and accessible to all of the
\GASPI{} processes forming the group \parameter{group}.  The
identifier \parameter{segment\_id} must be unique in the local
...
neither remotely. A subsequent call of \gaspifunction{segment\_use}
...
\begin{implementoradvice}
\gaspifunction{segment\_use} can be formulated in pseudo code as
...
\end{verbatim}
where the call gets executed on all members of \parameter{group}.
...
\parameter{segment\_max} in the configuration structure which is
retrieved by \gaspifunction{config\_get} and cannot exceed that
...
\parameterlistbegin
\parameterlistitem{in}{segment\_id}{the remote segment bound to the notification}
...
If the queue to which the notification request has been posted is full, i.\,e.\ if the number of
posted communication requests has already reached the queue size of a given queue, the notification
request has not been issued and the procedure returns with return value \GASPIQUEUE{}.
In this case users should either switch to another queue or wait (see \gaspifunction{wait}) and subsequently re-issue the notification request.
...
\parameterlistbegin
\parameterlistitem{in}{segment\_id}{the segment bound to the notification}
...
\parameterlistitem{out}{first\_id}{the id of the first notification that arrived}
\parameterlistitem{in}{timeout}{the timeout}
\parameterlistend
...
local side posts a \gaspifunction{read\_notify} call. \GASPI{} guarantees, that
if the notification has arrived on the local process, the posted read
request carrying the work load of the function \gaspifunction{read\_notify}
has arrived as well.
\end{useradvice}
...
\begin{implementoradvice}
The procedure is not semantically equivalent to a call to \gaspifunction{write\_list}
and a subsequent call of \gaspifunction{notify}. This call does not
enforce an ordering relative to other write operations.
\end{implementoradvice}
...
\subsubsection{\gaspifunction{read\_notify}}

The \gaspifunction{read\_notify} variant extends the simple
...
procedure \gaspifunction{waitsome}. It is an \gaspisemantic{asynchronous}
\gaspisemantic{non-local} \gaspisemantic{time-based blocking}
procedure.
...
\begin{useradvice}
In contrast to the procedure \gaspifunction{write\_notify}, the notification 
in the procedure \gaspifunction{read\_notify} carries the (fixed) notification value of 1.
Similar to the procedure \gaspifunction{write\_notify} a call to \gaspifunction{read\_notify}
...
\begin{useradvice}
The two \GASPI{} functions \gaspifunction{read\_notify} and  \gaspifunction{notify\_waitsome} establish a logical and thread safe happens-before relation between them.
\end{useradvice}

\begin{useradvice}
The notifcation driven \gaspifunction{read\_notify} complements the \gaspifunction{write\_notify} functionality.

While a \gaspifunction{read\_notify} features a variety of use cases (e.g. in distributed memory management)
one of the more remarkable goals of the function \gaspifunction{read\_notify} is to establish latency-tolerant multithreading in distributed memory systems.
...
(in first order) given by the product of the number of available queues and the queue depth ($queue\_num * queue\_size\_max$).
...
\insertlisting{readnotifylisting}{\gaspifunction{read\_notify} Example usage}
...
read operations. We note that the procedure \gaspifunction{read\_notify} 
...
\subsubsection{\gaspifunction{queue\_create}}
The \gaspifunction{queue\_create} procedure is a
\gaspisemantic{synchronous} \gaspisemantic{non-local}
\gaspisemantic{time-based blocking} procedure which creates a new
...
the communication
\parameter{queue} is created and available for communication requests
on it.
...
\gaspifunction{queue\_create} has to be performed in order to complete
...
\subsubsection{\gaspifunction{queue\_delete}}
The \gaspifunction{queue\_delete} procedure is a
\gaspisemantic{synchronous} \gaspisemantic{non-local}
\gaspisemantic{time-based blocking} procedure which deletes a given
queue.
...
After successful procedure completion, \ie return value \GASPISUCC{},
the communication
\parameter{queue} is deleted and no longer available for
communication. It is an application error to use the queue after
\gaspifunction{queue\_delete} has been invoked.
...
\gaspifunction{passive\_send} is the routine called by
...
\parameterlistitem{inout}{reduce\_state}{reduction state vector}
\parameterlistitem{in}{group}{the group of ranks which participate in the reduction operation}
\parameterlistitem{in}{timeout}{the timeout}
\parameterlistend
...
The fortran user defined callback function requires an  explicit type from the iso\_c\_binding module. in this
example integer(c\_int) (op\_one,op\_two,op\_res,op\_state).
...
\begin{TDef}
\TDefDefn{void*}{\gaspitype{reduce\_state}}
...
\insertlisting{../examples/allreduce_user.f90}{\GASPI{} User defined allreduce, fortran example. \label{lst:allreduce_user_f90}}
...
\subsubsection{\gaspifunction{queue\_max}}
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{out}{queue\_max}{the maximum number of allowed queues}
\parameterlistend
...
\subsubsection{\gaspifunction{atomic\_max}}
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{out}{max\_value}{the maximum value an gaspi\_atomic\_value\_t can hold}
\parameterlistend
...
\subsubsection{\gaspifunction{print\_error}}

The \gaspifunction{print\_error} procedure is a \gaspisemantic{synchronous}
\gaspisemantic{local} \gaspisemantic{blocking}
