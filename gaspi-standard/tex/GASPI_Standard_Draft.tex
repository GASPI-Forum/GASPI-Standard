...
shared variables and to synchronise processes or events.
...
\begin{verbatim}
gaspi_return_t
gaspi_segment_list ( gaspi_segment_id_t num
                   , gaspi_segment_id_t segment_id_list[num] )
\end{verbatim}
\end{FDefSign}
\parameterlistbegin
  \parameterlistitem{in}{num}{number of segment IDs to collect}
  \parameterlistitem{in}{segment\_list[num]}{list of locally allocated segment IDs}
\parameterlistend
\executionphasework
\FStdRetDescNOTimeout
\end{FDef}
...
has been posted to the given queue can be considered as completed on the local side.
...
posted request to the same process have arrived on the remote side.
...
\begin{useradvice}
  The parameter \parameter{tag} is \emph{not} used to match sends and
  reveices. It is just an additional information that the sender
  attaches to the data, e.\,g.\ the type. On the receiver side, \emph{all}
  messages are accepted, regardless of the value of the tag. However,
  by reading the submitted tag-value, the receiver can act differently
...
request which has been posted can be considered as completed on the local side.
...
The queue size is set to zero by a successful call to \gaspifunction{passive\_wait}.
...
is provided through three operations: set, fetch-and-add and compare-and-swap.
...
After successful procedure completion, i.\,e.\ return value \GASPISUCC{}, the parameter
\parameter{value\_old} contains the value of the atomic counter before the comparision
was done.
...
An implementation shall not provide a compile-time constant maximum for \gaspitype{statistic\_counter}.  
...
Thus a dynamically linked wrapper library can provide extra counters. 
...
In that case a wrapper library can be linked in order to retrieve statistical information from inside the application. 


\subsubsection{\gaspifunction{statistic\_counter\_info}}

The \gaspifunction{statistic\_counter\_info} procedure is a \gaspisemantic{synchronous}
\gaspisemantic{local} \gaspisemantic{blocking}
...
\parameterlistitem{out}{verbosity\_level}{minimum verbosity level to activate this counter}
\parameterlistend
\executionphaseany
\FStdRetDescNOTimeout
\end{FDef}
...
For instance, if a counter retrieves the bytes sent per rank, then \parameter{argument} contains the rank number.
After successful procedure completion, i.\,e.\ return value \GASPISUCC{}
