...
\begin{useradvice}\label{useradvice:gaspi_wait}
  ...
  requests inn this queue is in transfer to the remote side. It does not mean,
...
request carrying the work load of the function gaspi read notify
has arrived as well.
\end{useradvice}
...
enforce an ordering relative to preceding write operations.
\end{implementoradvice}
...
\begin{useradvice}
The two GASPI functions \gaspifunction{read\_notify} and  \gaspifunction{waitsome} establish a logical and thread safe happens-before relation between them.
\end{useradvice}
...
rather than local completion. Also this call does not enforce an ordering relative to preceding 
