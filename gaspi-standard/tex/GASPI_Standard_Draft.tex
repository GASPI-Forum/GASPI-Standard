...
\begin{todo}
\item short intro about weak synchronization
\end{todo}
...
\parameterlistitem{in}{flag}{the flag to be sent}
\parameterlistitem{in}{segment\_id\_flag}{the remote segment ID where the flag is sent to}
\parameterlistitem{in}{offset\_flag}{the remote offset where the flag is sent to}
\parameterlistitem{in}{queue}{the queue to be used}
\parameterlistitem{in}{timeout}{the timeout}
\parameterlistend
\executionphasework
...
\parameter{flag} of the local \GASPI{} process to a notification flag buffer of a
remote \GASPI{} process. This notification request is posted to the communication queue \parameter{queue}.
The remote notification flag buffer is specified by the triple \parameter{rank}, \parameter{segment\_id\_flag},
\parameter{offset\_flag}.
...
A valid \gaspifunction{notify} communication request requires that  the
remote segment is allocated, that there is a connection between the local
and the remote \GASPI{} process and that the remote segment has been registered
on the local \GASPI{} process. Otherwise, the communication request is invalid
and the procedure returns with \GASPIGERR{}.
...
gaspi_notify_waitsome ( gaspi_segment_id_t segment_id
                      ...
                      , gaspi_size_t   flag_buffer_size
                      , gaspi_timeout_t timeout
                      )
\end{verbatim}
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{in}{segment\_id}{the local segment ID where the flags reside}
\parameterlistitem{in}{offset\_begin}{the local offset where the first flag resides}
\parameterlistitem{in}{flag\_buffer\_size}{the size of the flag buffer}
\parameterlistitem{in}{timeout}{the timeout}
\parameterlistend
\executionphasework
\FStdRetDesc
...
pair \parameter{segment\_id}, \parameter{offset\_begin}
and \parameter{flag\_buffer\_size}. It contains $(\mbox{\it flag\_buffer\_size}
/\mbox{\it size\_flag}$ many consecutive
flags, where \emph{size\_flag} is the size of a single flag. This
value is given by the function \gaspifunction{notify\_size}.
% The first
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{in}{segment\_id}{the local segment ID where the flag to reset resides}
\parameterlistitem{in}{offset}{the local offset where the flag to reset resides}
...
\gaspifunction{notify\_reset} resets the flag which resides in the
local segment with ID \parameter{segment\_id} at \parameter{offset} to
...
The flag buffer on the local side is specified by the pair \parameter{segment\_id}, \parameter{offset}.
...
\subsubsection{\gaspifunction{notify\_size}}

Since the notification flag buffer passed to \gaspifunction{notify} and \gaspifunction{notify\_waitsome}
resides in a segment, the size of a notification flag
needs to be known in order to be able to manage the memory of the segment.
\gaspifunction{notify\_size} returns the size of a notification flag in units of bytes.
It is \gaspisemantic{synchronous} \gaspisemantic{local}
\gaspisemantic{blocking} procedure.
...
\end{FDefSign}
\parameterlistbegin
\parameterlistitem{out}{notify\_size}{size of flag in bytes}
\parameterlistend
\executionphasework
\FStdRetDescNOTimeout
\end{FDef}
...
After successful procedure completion, i.\,e.\ return value \GASPISUCC{}, \parameter{size\_flag}
contains the size of a notification flag in units of bytes.

If the return value is \GASPIGERR{}, \parameter{size\_flag} has an undefined value.
